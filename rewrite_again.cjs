...
const searchStr = `  const handleCreatePublicOrder = async (e: FormEvent) => {`;
const returnIndex = fileContent.lastIndexOf(searchStr);

if (returnIndex === -1) {
    console.log("Could not find handleCreatePublicOrder");
    process.exit(1);
}

// Keep the top part up to the end of handleCreatePublicOrder
const endOfHandleCreate = fileContent.indexOf('};', returnIndex) + 2;
let topPart = fileContent.substring(0, endOfHandleCreate);
